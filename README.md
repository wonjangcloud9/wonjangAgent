...
같은 박스 시스템으로 만드는 **공유 카드 4종**:

- 🌟 **자랑 카드** — `wonjang 자랑` (한 달의 습관·집중·지출·디데이)
- 🌱 **성장 카드** — `wonjang 성장` (원장이 기억한 사실·익힌 스킬·함께한 대화 — "쓸수록 성장"이 눈에 보이게)
...
| 갈래 | 할 수 있는 일 |
| --- | --- |
| 🤖 **AI 에이전트** | 자연어로 셸·파일·이메일·문서 작업. 시작 시 **Claude Code/Codex에 자동 연결**(키 불필요) — 연결 시 원장 비서 도구(기억·알림·가계부·날씨 등)도 **MCP로 함께 물려줌** |
| 🌟 **공유 카드** | 카톡에 안 깨지는 한 장 카드 4종 — **자랑 카드**(`자랑`, 주간 `--주`) · **성장 카드**(`성장`, 원장이 배운 것) · **D-day 카드**(`디데이 카드`, 수능 D-100) · **기념일 카드**(`기념일`, 우리 1000일). 복사 `--복사`·카톡폭 `--폭 34` |
...
| 🇰🇷 **실시간 한국 정보** | 지하철 · 혼잡도 · 따릉이 · 날씨 · 미세먼지 · 환율 · 코인 · 뉴스 · 긱뉴스 · 공휴일(**다음 연휴·연차 하나로 황금연휴 + 연휴 공유 카드 `--카드`**) |
...
- 🧠 **영속 메모리** — 사용자/환경에 대해 배운 사실을 디스크에 저장하고 다음
  세션에 자동으로 불러와 "함께 성장"합니다. `wonjang 기억 "<사실>"`로 직접
  기억시키고, 틀린 기억은 `wonjang 잊어 <키워드>`(또는 대화로 "그거 잊어줘")로
  지웁니다.
- 📚 **스킬(절차 기억)** — 까다로운 작업을 해결하면 그 방법을 스킬로 저장하고,
  비슷한 일을 만나면 스스로 꺼내 재사용합니다. 잘못 저장된 스킬은
  `wonjang 스킬 삭제 <이름>`으로 지웁니다.
- 🌱 **성장 카드** — `wonjang 성장` 한 번이면 기억한 사실·익힌 스킬·함께한
  대화 수와 "함께 N일째"를 한 장 카드로 — 성장이 눈에 보입니다.
...
```
  원장 에이전트  v1.67.1
  🔑 키 없이 Claude Code에 연결됐어요 — 바로 쓸 수 있어요
  안녕하세요, 원장입니다 🌙  오늘은 6월 11일 (목).
  🔥 운동 5일째   ▓▓░▓▓▓▓
  🧠 기억 4 · 스킬 1 — 점점 당신에게 맞춰지는 중 (wonjang 성장)
  💡 오늘의 팁: 이번 달 자랑 카드 → wonjang 자랑 (카톡엔 --폭 34)
  무엇이든 한국어로 시켜보세요.  /help · /성격 · /자랑 · /exit
...
초기 로드맵(에이전트 코어 → 한국 생활 유틸 100여 종)을 모두 달성하고, 지금은 매 릴리스마다 매력·완성도를 다듬고 있습니다. **버전별 변경 이력은 [CHANGELOG.md](CHANGELOG.md)에서** 한눈에 볼 수 있습니다.
