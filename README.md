...
| 갈래 | 할 수 있는 일 |
| --- | --- |
| 🤖 **AI 에이전트** | 자연어로 셸·파일 작업. 시작 시 **Claude Code/Codex에 자동 연결**(키 불필요) |
| 📂 **내 파일 다루기** | 엑셀·CSV 분석 · 용량 분석 · 중복 찾기 · 자동 분류 · 이름 일괄변경 · 압축 · 내용검색 · JSON · 체크섬 |
| 🇰🇷 **실시간 한국 정보** | 지하철 · 혼잡도 · 따릉이 · 날씨 · 미세먼지 · 환율 · 코인 · 뉴스 · 긱뉴스 · 공휴일 |
| 🗓️ **개인 비서** | 약속·알림 · 할일 · 디데이 · 가계부 · 습관 · 집중타이머 · 즐겨찾기 · 아침 브리핑 · 푸시알림 |
| 🧮 **생활·금융 계산기** | 실수령액 · 대출 · 예적금 · 평수 · 만나이 · 할인 · 부가세 · 시급 · 단위변환 · 그 외 다수 |
| 🛠️ **편의** | QR 생성 · 또간집 맛집 · 메뉴추천 · 제비뽑기 · 옵시디언/노션 연동 |

> 대부분 **키 없이 바로** 동작합니다. 일부 실시간 정보는 무료 키로 더 풍부해집니다(`wonjang config` 참고).
...
- 🔖 **즐겨찾기 + 빠른 열기** — `wonjang 열기 노션`처럼 자주 가는 사이트·폴더·앱을
  한국어 단축어로 엽니다.
- 📊 **엑셀·CSV 분석** — `wonjang 엑셀 매출.csv`로 **내 컴퓨터의 파일을 직접 읽어**
  요약·미리보기를, `--열 금액`으로 합계·평균·최대·최소를, `--json`으로 JSON 변환을 합니다(.xlsx도 지원).
- 🍜 **또간집 맛집 찾기** — `wonjang 또간집 종로`로 풍자 〈또간집〉 선정 맛집을 지역으로
  검색합니다. 공식 API가 없어 직접 키우는 로컬 목록(`--추가`로 확인한 곳 등록).
- 💾 **디스크 용량 분석** — `wonjang 용량 ~/Downloads`로 큰 파일·폴더를 찾아
  "어디가 꽉 찼는지"를 알려줍니다(읽기 전용, 안전).
- 🔁 **중복 파일 찾기** — `wonjang 중복 ~/Downloads`로 내용이 같은 파일과 낭비 용량을
  찾아줍니다(크기+내용 해시 비교, 읽기 전용).
- 🗂️ **폴더 자동 분류** — `wonjang 정리 ~/Downloads`로 뒤섞인 파일을 이미지·문서·동영상
  등으로 분류합니다. **기본은 미리보기**, `--실행`을 줘야 실제로 옮깁니다(안전).
- ✏️ **파일 이름 일괄 변경** — `wonjang 이름변경 ~/사진 IMG_ 여행_`으로 파일명 속 문자를
  한 번에 치환합니다(기본 미리보기, `--실행`으로 적용).
- 🗜️ **압축/해제** — `wonjang 압축 ~/문서`로 zip을 만들고 `wonjang 압축풀기 자료.zip`으로
  풉니다(한글 파일명·폴더 구조 보존, zip-slip 방어).
- 🔎 **파일 내용 검색** — `wonjang 찾기 ~/메모 계약`으로 폴더 안 텍스트 파일에서 단어가 든
  줄을 파일·줄번호와 함께 찾습니다(바이너리 제외, 읽기 전용).
- 🧩 **JSON 도구** — `wonjang json data.json`으로 검증·정렬을, `--키 meta.port`로 값을
  추출합니다(오류 위치 표시).
- 🔐 **파일 체크섬** — `wonjang 해시 setup.dmg`로 SHA-256을 계산하고 `--확인 <값>`으로
  다운로드 파일 무결성을 검증합니다(shasum과 동일).
- 📑 **파일 비교(diff)** — `wonjang 비교 old.txt new.txt`로 두 파일의 줄 단위 차이(추가/삭제)를
  봅니다(설정·버전 비교).
- 📱 **QR 코드 생성** — `wonjang qr https://...` 또는 `wonjang qr --wifi <SSID> --비번 <비번>`으로
  터미널에 스캔 가능한 QR을 그립니다(와이파이·링크 공유).
- 🌏 **세계 시간** — `wonjang 세계시간 뉴욕`으로 주요 도시의 현재 시각을 봅니다(서머타임 정확 반영).
- 🤓 **긱뉴스(GeekNews)** — `wonjang 긱뉴스`로 개발·기술·스타트업 최신글을 봅니다(키 불필요).
- 🐙 **GitHub 저장소 정보** — `wonjang 깃헙 rust-lang/rust`로 별·이슈·최신 릴리스를 봅니다(키 불필요).
- 🌐 **내 공인 IP** — `wonjang 내아이피`로 현재 공인 IP·통신사·위치를 확인합니다(VPN·네트워크 점검).
- 🩺 **사이트 상태 확인** — `wonjang 사이트 example.com`으로 사이트/서버가 살아있는지·응답 속도를 점검합니다.
- 🗓️ **한국 공휴일** — `wonjang 공휴일`로 설날·추석(음력)까지 정확한 공휴일과
  "다음 빨간날 D-day"를 봅니다(무료 API, 키 불필요). 연차 계획에 유용.
- 🚇 **서울 지하철 실시간** — `wonjang 지하철 강남`으로 역 이름만 입력하면 실시간
  도착정보를 보여줍니다(샘플 키로 바로 동작).
- 🚦 **서울 실시간 혼잡도** — `wonjang 혼잡도 강남역`으로 명소·상권의 붐빔 정도와 실시간
  인구를 봅니다(서울 열린데이터 키 발급 시 원하는 지역 조회).
- 🚲 **서울 따릉이 실시간** — `wonjang 따릉이 강남역`으로 대여소의 남은 자전거·거치대를
  조회합니다(혼잡도·지하철과 같은 서울 키 슬롯).
...
- ⏰ **유닉스 타임스탬프** — `wonjang 타임스탬프`로 현재 유닉스 시각을, `1700000000`으로 로그 타임스탬프를 사람이 읽는 시각으로 변환합니다.
- 🔠 **인코딩** — `wonjang 인코딩 base64 "텍스트"`(또는 `url`)로 인코딩·디코딩(`-d`)합니다(JWT·URL 파라미터).
- 🔑 **안전한 비밀번호 생성** — `wonjang 비번 16 --기호`로 OS 난수 기반 강력한 비밀번호를(암호학적으로 안전).
- 🆔 **UUID 생성** — `wonjang uuid -n 3`으로 무작위 UUID v4를 생성합니다(DB 기본키·요청 ID).
- 🎨 **색상 변환** — `wonjang 색 #ff5733`으로 HEX↔RGB↔HSL을 변환합니다(디자인·CSS).
...
- [x] v0.66 — 엑셀·CSV 분석(`wonjang 엑셀`, 로컬 파일 직접 읽기)
- [x] v0.67 — 또간집 맛집 찾기(`wonjang 또간집 <지역>`, 직접 키우는 목록)
- [x] v0.68 — 디스크 용량 분석(`wonjang 용량`, 큰 파일·폴더 찾기)
- [x] v0.69 — 중복 파일 찾기(`wonjang 중복`, 내용 해시 비교)
- [x] v0.70 — 폴더 자동 분류(`wonjang 정리`, 미리보기→`--실행`)
- [x] v0.71 — 파일 이름 일괄 변경(`wonjang 이름변경`, 미리보기→`--실행`)
- [x] v0.72 — 한국 공휴일 조회(`wonjang 공휴일`, 설날·추석 포함, 키 불필요)
- [x] v0.73 — 서울 실시간 혼잡도(`wonjang 혼잡도`, k-skill 참고)
- [x] v0.74 — 긱뉴스(`wonjang 긱뉴스`, 개발 뉴스, k-skill 참고)
- [x] v0.75 — 서울 따릉이 실시간(`wonjang 따릉이`, k-skill 참고)
- [x] v0.76 — 폴더 압축/해제(`wonjang 압축`/`압축풀기`, zip)
- [x] v0.77 — QR 코드 생성(`wonjang qr`, URL·와이파이)
- [x] v0.78 — 파일 내용 검색(`wonjang 찾기`, grep)
- [x] v0.79 — JSON 도구(`wonjang json`, 검증·정렬·값 추출)
- [x] v0.80 — 파일 체크섬(`wonjang 해시`, SHA-256/512)
- [x] v0.81 — 파일 비교(`wonjang 비교`, 줄 단위 diff)
- [x] v0.82 — 엑셀/CSV → JSON 변환(`wonjang 엑셀 --json`)
- [x] v0.83 — GitHub 저장소 정보(`wonjang 깃헙`, 별·이슈·릴리스)
- [x] v0.84 — 내 공인 IP(`wonjang 내아이피`, IP·통신사·위치)
- [x] v0.85 — 사이트 상태 확인(`wonjang 사이트`, 응답 코드·속도)
- [x] v0.86 — 유닉스 타임스탬프 변환(`wonjang 타임스탬프`)
- [x] v0.87 — base64/URL 인코딩·디코딩(`wonjang 인코딩`)
- [x] v0.88 — 안전한 비밀번호 생성(`wonjang 비번`, OS 난수)
- [x] v0.89 — UUID v4 생성(`wonjang uuid`)
- [x] v0.90 — 색상 변환(`wonjang 색`, HEX↔RGB↔HSL)
- [x] v0.91 — 세계 시간(`wonjang 세계시간`, DST 정확)
- [ ] 이메일(읽기/요약/발송) — IMAP 앱 비밀번호 필요
- [ ] 최저가·항공권 검색 — 네이버 쇼핑/항공 API 키 필요
- [ ] 또간집 정확 데이터 — 카카오 로컬 API 키 또는 공유 리스트
