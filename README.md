...
## 🌟 자랑 카드 — 한 달의 나를 카톡에 자랑

매일 쌓은 **습관 잔디·집중·지출·디데이·일기**를, 카톡 코드블록에서도 안 깨지는
**한 장 카드**로 만듭니다. `wonjang 자랑` 한 번이면 끝(주간은 `--주`, 클립보드 복사는 `--복사`).
...
> PNG가 아니라 **텍스트**라 어디든 붙여넣기만 하면 됩니다. 한글·이모지의 전각폭을
> 정확히 계산해 박스가 절대 어긋나지 않습니다(폭은 `--폭 34`로 카톡에 맞춤).

같은 박스 시스템으로 만드는 **공유 카드 3종**:

- 🌟 **자랑 카드** — `wonjang 자랑` (한 달의 습관·집중·지출·디데이)
- 🎯 **D-day 카드** — `wonjang 디데이 카드` ("수능 D-100"을 캡처해 자랑)
- 💕 **기념일 카드** — `wonjang 기념일 2024-01-01 우리` ("우리 1000일째")

모두 풋터 `나도 만들기 → wonjang-agent`로 받은 사람도 바로 설치할 수 있어요.
...
| 갈래 | 할 수 있는 일 |
| --- | --- |
| 🤖 **AI 에이전트** | 자연어로 셸·파일·이메일·문서 작업. 시작 시 **Claude Code/Codex에 자동 연결**(키 불필요) |
| 🌟 **공유 카드** | 카톡에 안 깨지는 한 장 카드 3종 — **자랑 카드**(`자랑`, 주간 `--주`) · **D-day 카드**(`디데이 카드`, 수능 D-100) · **기념일 카드**(`기념일`, 우리 1000일). 복사 `--복사`·카톡폭 `--폭 34` |
| 🎭 **내 비서의 성격** | `wonjang 성격 친구`로 말투를 고릅니다(다정·친구·집사·선배·발랄). SOUL.md로 직접 편집 가능 |
| 📧 **이메일** | 받은편지함 **목록·검색·본문 읽기 · 첨부 저장 · 보내기(파일 첨부)**. IMAP/SMTP(순수 rustls), 앱 비밀번호만 있으면 OK |
| 📄 **PDF·이미지** | 사진→PDF **묶기** · PDF **합치기/페이지 추출/회전** · 이미지 **축소·압축·형식변환(png↔jpg)·이어붙이기(긴 캡처)**, EXIF 방향 자동 보정 |
| 📂 **내 파일 다루기** | 엑셀·CSV 분석 · 용량/중복/자동분류 · 이름 일괄변경 · 압축(**내용 보기** 포함) · **한글 깨짐(CP949) 복구** · 내용검색 · JSON · 체크섬 |
| 🇰🇷 **실시간 한국 정보** | 지하철 · 혼잡도 · 따릉이 · 날씨 · 미세먼지 · 환율 · 코인 · 뉴스 · 긱뉴스 · 공휴일(**다음 연휴·연차 하나로 황금연휴**) |
| 🗓️ **개인 비서** | 약속·알림 · 할일 · 디데이(**캘린더 .ics 내보내기**) · 가계부(**CSV 내보내기→분석**) · 습관 · 집중타이머 · 즐겨찾기 · 일기 · 아침 브리핑 · 푸시알림 |
| 🧮 **생활·금융 계산기** | 실수령액 · 대출 · 예적금 · **전월세 전환** · **퇴직금·연차·야근수당**(근로기준법) · **자동차세** · 평수 · 만나이(**살아온 날·기념일**) · **한글이름 → 여권 로마자** · 할인 · 부가세 · 시급 · 자소서 글자수(`--제한`) · 단위변환(**돈·근·자 등 한국 전통단위**) · 그 외 다수 |
...
- ⏰ **약속·알림** — "내일 3시 치과 알려줘"처럼 등록하면, 24시간 스케줄러가
  ...
  (`wonjang remind`).
- ✅ **할 일 관리** — `wonjang 할일 추가 "장보기"`로 체크리스트 관리. 약속(시각)과
  할 일(목록)을 따로 관리합니다.
- ☀️ **아침 브리핑** — `브리핑` 프리셋으로 오늘 날짜·날씨·약속·할 일을 한 번에.
- 📲 **푸시 알림** — 디스코드/슬랙/텔레그램/**카카오톡(나에게 보내기)**으로 알림을
...
- 💰 **가계부** — `wonjang 지출 추가 8000 식비 점심`으로 지출을 기록하고
  오늘·이번달 합계, 분류별 지출을 봅니다.
- 🔥 **습관 트래커** — `wonjang 습관 추가 "운동"` / `완료`로 매일 체크하고
...
- 📊 **엑셀·CSV 분석** — `wonjang 엑셀 매출.csv`로 **내 컴퓨터의 파일을 직접 읽어**
  요약·미리보기를, `--열 금액`으로 합계·평균·최대·최소를, **`--그룹 지점 --열 매출`으로
  분류별 집계(피벗·막대그래프)**를, **`--정렬 매출`로 상위 행**을, **`--필터 지역=서울`로 조건 행만**(정렬·집계와 조합)을,
  `--json`으로 JSON 변환을 합니다. **여러 시트가 있는 .xlsx는 `--시트 재고`로 탭을 골라** 분석하고,
  **`--저장 결과.csv`로 거른·정렬한·집계한 결과를 새 CSV로 내보냅니다**(읽기→분석→저장 한 줄).
- 🧩 **여러 표 합치기** — `wonjang 표합치기 1월.csv 2월.csv 3월.csv --저장 합본.csv`로 월별·지점별
  파일을 **머리글 기준으로 한 장에**(열 순서가 달라도 이름으로 맞춤, `--출처`로 출처 열 추가). 합친 뒤 바로 `엑셀`로 분석.
...
- 🗜️ **압축/해제·미리보기** — `wonjang 압축 ~/문서`로 zip을 만들고 `wonjang 압축풀기 자료.zip`으로
  풉니다(한글 파일명·폴더 구조 보존, zip-slip 방어). `wonjang 압축보기 자료.zip`은 **풀지 않고 목록만**
  보여주며, 윈도우 zip의 **CP949로 깨진 한글 파일명을 자동 복구**합니다.
- 📧 **이메일(IMAP·SMTP)** — 앱 비밀번호만 환경변수(`WONJANG_EMAIL`·`WONJANG_EMAIL_PASSWORD`)에
  넣으면, `wonjang 메일`(받은편지함)·`메일검색 영수증`·`메일읽기 1`(본문)·`메일첨부 1`(첨부 저장)·
  `메일보내기 --받는사람 … --첨부 파일`(보내기)까지. **순수 rustls**라 openssl 없이 동작하고,
  한글 제목·CP949까지 제대로 디코드합니다. (gmail·naver·daum·kakao 등 호스트 자동 추정)
- 📄 **PDF 다루기** — `wonjang 사진묶기 *.jpg`로 여러 사진을 **한 PDF로**(서류 제출),
  `pdf합치기 a.pdf b.pdf`(합본) · `pdf페이지 보고서.pdf 1-3,5`(원하는 페이지만) ·
  `pdf회전 스캔.pdf 90`(옆으로 스캔된 서류 바로 세우기). 모두 로컬에서, 키 없이.
- 🖼️ **이미지 다루기** — `wonjang 이미지 *.jpg --폭 1280`으로 **여러 장을 한 번에** 줄여
  첨부 용량을 낮추고(폰 사진의 **EXIF 방향 자동 보정**), `--형식 jpg`/`--형식 png`로
  png↔jpg를 변환합니다. **`wonjang 이미지이어붙이기 1.png 2.png --세로`로 여러 캡처·영수증을 한 장으로**(가로는 `--가로`).
  원본은 항상 보존(`_작게`/`_변환`).
...
- 🕑 **시간대 변환** — `wonjang 시차 09:00 서울 뉴욕`으로 특정 시각을 도시 간 환산합니다(해외 회의 일정).
...
- 📏 **단위 변환** — `wonjang 변환 100 c`로 온도·무게·길이·속도·부피·넓이를 변환합니다(c/f, kg/lb, cm/inch, km/mile, kmh/mph, l/gal, sqm/sqft).
- ⚖️ **BMI 계산** — `wonjang bmi 175 68`로 체질량지수와 판정·표준체중을(대한비만학회 아시아 기준).
- 🍚 **칼로리 계산** — `wonjang 칼로리 남 30 175 70`으로 기초대사량(BMR)·활동별 하루 권장 칼로리를(다이어트·운동).
- 😴 **수면 시간 계산** — `wonjang 수면 07:00`으로 90분 수면 주기에 맞춘 개운한 취침/기상 시각을.
...
- 🏛️ **로마 숫자** — `wonjang 로마 2024`(→MMXXIV) 또는 `wonjang 로마 MMXXIV`(→2024)로 양방향 변환합니다.
...
- ☀️ **자동 아침 브리핑** — 스케줄러가 매일 정해진 시각에 브리핑을 만들어
  휴대폰(카카오/디스코드/슬랙/텔레그램)으로 자동 푸시합니다(`WONJANG_BRIEFING_TIME`).
...
- [x] v0.92 — 시간대 변환(`wonjang 시차`, 도시 간 시각 환산)
- [x] v0.93 — 간단 일기(`wonjang 일기`, 월별 로컬 저장)
- [x] v0.94 — 단위 변환 확장(속도·부피·넓이 추가)
- [x] v0.95 — 칼로리 계산(`wonjang 칼로리`, BMR·TDEE)
- [x] v0.96 — 수면 시간 계산(`wonjang 수면`, 90분 주기)
- [x] v0.97 — 로마 숫자 변환(`wonjang 로마`, 숫자↔로마자)
- [x] v0.98 — 에이전트 페르소나(`wonjang 성격`, SOUL.md) — "내 비서" 경험
- [x] v0.99 — 첫인상 배너 재설계(키 없이 연결됨 자랑 + 성격 인사 + 오늘 날짜)
- [x] v1.00 — 첫 실행 온보딩 + 개인화 브리핑(매력 P0~P1 완성)
- [x] v1.01 — 선제 알림: 공휴일 전날 "내일 빨간날" 자동 푸시(데몬)
