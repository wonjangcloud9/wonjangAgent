...
pub fn builtin() -> Vec<Preset> {
    vec![
        ...
        Preset::new(
            ...
            &["briefing", "아침브리핑"],
            "사용자를 위한 오늘의 아침 브리핑을 만들어줘. **당신의 성격(말투)을 유지**하며, \
             단순 정보 나열이 아니라 '어제의 너를 기억하고 오늘 먼저 챙겨주는' 따뜻한 비서처럼 써.\
             \n\n[먼저 인사 + 어제 돌아보기] 성격에 맞는 인사 한마디 뒤에, 어제 기록을 참고해 \
             개인화된 한 줄을 더해줘: 어제 일기('wonjang 일기'에서 최근 기록), 이번 달 지출 \
             ('wonjang 지출'), 습관 연속일수('wonjang 습관'의 streak). 기록이 있으면 자연스럽게 \
             언급하고(예: '어제 ~라고 적으셨네요', '이번 달 식비가 좀 늘었어요', '운동 5일 연속! 대단해요'), \
             없으면 생략해.\
             \n\n[오늘 챙길 것] 간결한 불릿으로:\
             \n- 오늘 날짜·요일('date')\
             \n- 서울 날씨(weather_now 또는 'wonjang 날씨')·미세먼지(air_quality) — 나쁘면 꼭 알려줘\
             \n- 예정 약속·알림('wonjang remind', 가까운 순)\
             \n- 오늘 할 일('wonjang todo')\
             \n- 가까운 디데이('wonjang dday')\
             \n- 아직 안 한 습관('wonjang 습관') — streak 끊기지 않게 콕 집어줘\
             \n- 다음 공휴일 D-day('wonjang 공휴일'). 내일이 공휴일이면 '내일 빨간날이에요!'라고 강조\
             \n- 주요 뉴스 2~3개('wonjang 뉴스')\
             \n\n[마무리] 성격에 맞는 짧은 응원 한마디.",
        ),
        Preset::new(
            "회고",
