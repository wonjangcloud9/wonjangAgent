...
pub fn system_prompt(memory_block: Option<String>, skills_block: Option<String>) -> String {
    ...
    let mut prompt = format!(
        "{persona}\n\n\
         ...
         remember 도구가 없는 환경이면 셸로 `{exe} 기억 \"<사실>\"`을 실행해도 같습니다. \
         기억이 틀렸거나 사용자가 \"그거 잊어줘\"라고 하면 forget 도구(또는 `{exe} 잊어 <키워드>`)로 지우세요.\n\
         ...
         - 성장 카드: `{exe} 성장`(당신이 remember로 기억한 사실·save_skill로 익힌 스킬·함께한 대화 수를 한 장 카드로 — 쓸수록 성장하는 걸 보여줌, `--복사`·`--폭 34`). \"너 나에 대해 뭐 알아?\"/\"성장 카드 보여줘\"\n\
         ...
         - 습관 체크: `{exe} 습관 add \"<이름>\"`, `{exe} 습관 done <이름>`, `{exe} 습관`(연속일수+최근 7일 잔디), `{exe} 습관 통계 <이름>`(달성률·최장 연속·요일별 패턴·가장 꾸준한 요일)\n\
         ...
         - 한국 공휴일: `{exe} 공휴일 [년도]` (설날·추석 포함, 다음 빨간날 D-day, **다음 연휴 며칠 연속** + **연차 하나로 만드는 황금연휴** 추천까지 — 대체공휴일·주말 끼임 정확 계산). **연휴 공유 카드**: `{exe} 공휴일 --카드`(다음 연휴 D-day를 카톡 캡처용 한 장으로, `--복사`·`--폭 34`). \"이번 추석 며칠 쉬어\"·\"연차 어떻게 쓰면 황금연휴\"·\"연휴 카드 만들어줘\". web_search보다 정확\n\
         - 생활·금융 계산기(정확한 사용법은 `{exe} 도움` 참고): 평수 `{exe} 평`, 만나이 `{exe} 나이`, 실수령(연봉 기본, \"월급 300이면 실수령\"→`{exe} 실수령 300 --월급`) `{exe} 실수령`, 시급 `{exe} 시급`, 대출 `{exe} 대출`, 예적금 `{exe} 예금`/`{exe} 적금`, 전월세전환(전세→반월세 월세, \"전세 3억 월세로 돌리면\"→`{exe} 전월세 30000 5.5 [보증금만원]`) `{exe} 전월세`, 퇴직금(법정 퇴직금 추정, \"3년 일하고 월급 300이면 퇴직금\"→`{exe} 퇴직금 300 3 [개월]`) `{exe} 퇴직금`, 연차(근로기준법 연차휴가 일수, \"5년차 연차 며칠\"→`{exe} 연차 5`) `{exe} 연차`, 자동차세(비영업 승용, \"내 차 1998cc 자동차세\"→`{exe} 자동차세 1998 [차령]`, 지방교육세·차령경감 포함) `{exe} 자동차세`, 야근수당(연장·야간·휴일 가산, \"시급 12000에 야근 3시간 수당\"→`{exe} 야근수당 12000 --연장 3 [--야간 N] [--휴일 N]`) `{exe} 야근수당`, 할인 `{exe} 할인`, 부가세 `{exe} 부가세`, 마진율(원가·판매가→마진율·마크업, \"30% 남기려면 얼마에 팔지\"→`{exe} 마진 7000 --목표 30`) `{exe} 마진`, 날짜·기념일(\"사귄 지 며칠\"·\"100일 언제\"→`{exe} 날짜 <사귄날>`, 며칠째+다음 기념일 양력날짜) `{exe} 날짜`, 기념일 공유 카드(\"우리 1000일 카드 만들어줘\"→`{exe} 기념일 <사귄날> [이름]`, 카톡에 캡처해 자랑·`--복사`·`--폭 34`) `{exe} 기념일`, 시간 `{exe} 시간`, 타임스탬프 `{exe} 타임스탬프`, 진법 `{exe} 진법`, 로마숫자 `{exe} 로마`, 로마자(한글 이름→여권 영문이름, \"홍길동 영문으로\"→`{exe} 로마자 홍길동`, 성은 관용표기) `{exe} 로마자`, 인코딩(base64/url) `{exe} 인코딩`, 색상변환 `{exe} 색`, 단위변환(온도/무게/길이/속도/부피/넓이 + **한국 전통단위 돈·근·관·되·말·자·치**, 예: \"금 5돈 몇 g\"→`{exe} 변환 5 돈`, \"이불 7자 몇 cm\"→`{exe} 변환 7 자`) `{exe} 변환`, BMI `{exe} bmi`, 칼로리(기초대사량) `{exe} 칼로리`, 수면시각(90분주기) `{exe} 수면`, 글자수(자소서 제한 체크 \"1000자 중 몇 자\"→`{exe} 글자수 \"...\" --제한 1000`, 공백 포함/제외 둘 다 남은 글자) `{exe} 글자수`, 초성 `{exe} 초성`, 한영타 `{exe} 영타`/`{exe} 한타`, 한글금액 `{exe} 금액`, 사칙연산 `{exe} 계산`, 더치페이 `{exe} 더치`, 제비뽑기 `{exe} 뽑기`, 비밀번호생성 `{exe} 비번`, UUID `{exe} uuid`, 메뉴추천 `{exe} 메뉴`, 로또 `{exe} 로또`\n\
