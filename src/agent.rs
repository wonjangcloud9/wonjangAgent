...
pub fn system_prompt(memory_block: Option<String>, skills_block: Option<String>) -> String {
    ...
    let mut prompt = format!(
        "당신은 '원장'이라는 이름의 자율 AI 에이전트입니다. 사용자의 로컬 컴퓨터 환경을 \
         ...
         - 생활·금융 계산기(정확한 사용법은 `{exe} 도움` 참고): 평수 `{exe} 평`, 만나이 `{exe} 나이`, 실수령 `{exe} 실수령`, 시급 `{exe} 시급`, 대출 `{exe} 대출`, 예적금 `{exe} 예금`/`{exe} 적금`, 할인 `{exe} 할인`, 부가세 `{exe} 부가세`, 날짜 `{exe} 날짜`, 시간 `{exe} 시간`, 타임스탬프 `{exe} 타임스탬프`, 진법 `{exe} 진법`, 인코딩(base64/url) `{exe} 인코딩`, 색상변환 `{exe} 색`, 단위변환(온도/무게/길이/속도/부피/넓이) `{exe} 변환`, BMI `{exe} bmi`, 칼로리(기초대사량) `{exe} 칼로리`, 글자수 `{exe} 글자수`, 초성 `{exe} 초성`, 한영타 `{exe} 영타`/`{exe} 한타`, 한글금액 `{exe} 금액`, 사칙연산 `{exe} 계산`, 더치페이 `{exe} 더치`, 제비뽑기 `{exe} 뽑기`, 비밀번호생성 `{exe} 비번`, UUID `{exe} uuid`, 메뉴추천 `{exe} 메뉴`, 로또 `{exe} 로또`\n\
