...
pub fn system_prompt(memory_block: Option<String>, skills_block: Option<String>) -> String {
    ...
    let mut prompt = format!(
        "{persona}\n\n\
         ...
         - 생활·금융 계산기(정확한 사용법은 `{exe} 도움` 참고): 평수 `{exe} 평`, 만나이 `{exe} 나이`, 실수령 `{exe} 실수령`, 시급 `{exe} 시급`, 대출 `{exe} 대출`, 예적금 `{exe} 예금`/`{exe} 적금`, 전월세전환(전세→반월세 월세, \"전세 3억 월세로 돌리면\"→`{exe} 전월세 30000 5.5 [보증금만원]`) `{exe} 전월세`, 퇴직금(법정 퇴직금 추정, \"3년 일하고 월급 300이면 퇴직금\"→`{exe} 퇴직금 300 3 [개월]`) `{exe} 퇴직금`, 연차(근로기준법 연차휴가 일수, \"5년차 연차 며칠\"→`{exe} 연차 5`) `{exe} 연차`, 할인 `{exe} 할인`, 부가세 `{exe} 부가세`, 날짜·기념일(\"사귄 지 며칠\"·\"100일 언제\"→`{exe} 날짜 <사귄날>`, 며칠째+다음 기념일 양력날짜) `{exe} 날짜`, 시간 `{exe} 시간`, 타임스탬프 `{exe} 타임스탬프`, 진법 `{exe} 진법`, 로마숫자 `{exe} 로마`, 인코딩(base64/url) `{exe} 인코딩`, 색상변환 `{exe} 색`, 단위변환(온도/무게/길이/속도/부피/넓이 + **한국 전통단위 돈·근·관·되·말**, 예: \"금 5돈 몇 g\"→`{exe} 변환 5 돈`, \"고기 두 근\"→`{exe} 변환 2 근`) `{exe} 변환`, BMI `{exe} bmi`, 칼로리(기초대사량) `{exe} 칼로리`, 수면시각(90분주기) `{exe} 수면`, 글자수(자소서 제한 체크 \"1000자 중 몇 자\"→`{exe} 글자수 \"...\" --제한 1000`, 공백 포함/제외 둘 다 남은 글자) `{exe} 글자수`, 초성 `{exe} 초성`, 한영타 `{exe} 영타`/`{exe} 한타`, 한글금액 `{exe} 금액`, 사칙연산 `{exe} 계산`, 더치페이 `{exe} 더치`, 제비뽑기 `{exe} 뽑기`, 비밀번호생성 `{exe} 비번`, UUID `{exe} uuid`, 메뉴추천 `{exe} 메뉴`, 로또 `{exe} 로또`\n\
