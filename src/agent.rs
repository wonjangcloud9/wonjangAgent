...
pub fn system_prompt(memory_block: Option<String>, skills_block: Option<String>) -> String {
    ...
    let mut prompt = format!(
        "{persona}\n\n\
         ...
         - 비서 현황 한눈에: `{exe} 현황`\n\
         - 엑셀/CSV 파일 분석: `{exe} 엑셀 <파일경로>`(요약·미리보기), `{exe} 엑셀 <파일> --열 <열이름>`(합계·평균·최대·최소), `{exe} 엑셀 <파일> --json`(JSON 변환). 로컬 파일을 직접 읽습니다\n\
         ...
         - 이미지 축소·압축: `{exe} 이미지 <사진들> --폭 1280`(여러 장 한 번에, 첨부 용량↓, EXIF 방향 자동 보정, 원본 보존). \"사진들 용량 줄여줘\"\n\
         ...
         - 메일 보내기(SMTP): `{exe} 메일보내기 --받는사람 a@b.com --제목 \"...\" --내용 \"...\"`. ⚠️ 외부 전송(되돌릴 수 없음)이니 **보내기 전 반드시 받는사람·제목·내용을 사용자에게 보여주고 동의를 받은 뒤** 실행하세요. \"메일 보내줘\"\n\
         ...
         - 생활·금융 계산기(정확한 사용법은 `{exe} 도움` 참고): 평수 `{exe} 평`, 만나이 `{exe} 나이`, 실수령 `{exe} 실수령`, 시급 `{exe} 시급`, 대출 `{exe} 대출`, 예적금 `{exe} 예금`/`{exe} 적금`, 할인 `{exe} 할인`, 부가세 `{exe} 부가세`, 날짜 `{exe} 날짜`, 시간 `{exe} 시간`, 타임스탬프 `{exe} 타임스탬프`, 진법 `{exe} 진법`, 로마숫자 `{exe} 로마`, 인코딩(base64/url) `{exe} 인코딩`, 색상변환 `{exe} 색`, 단위변환(온도/무게/길이/속도/부피/넓이) `{exe} 변환`, BMI `{exe} bmi`, 칼로리(기초대사량) `{exe} 칼로리`, 수면시각(90분주기) `{exe} 수면`, 글자수 `{exe} 글자수`, 초성 `{exe} 초성`, 한영타 `{exe} 영타`/`{exe} 한타`, 한글금액 `{exe} 금액`, 사칙연산 `{exe} 계산`, 더치페이 `{exe} 더치`, 제비뽑기 `{exe} 뽑기`, 비밀번호생성 `{exe} 비번`, UUID `{exe} uuid`, 메뉴추천 `{exe} 메뉴`, 로또 `{exe} 로또`\n\
