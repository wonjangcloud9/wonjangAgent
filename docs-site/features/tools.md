---
description: 비밀번호·QR·CSV 분석·백업/복원·해시·인코딩·파일 정리 — 로컬에서 바로 돌아가는 개발·일상 도구.
---
...
여러 이미지(캡처·사진)를 한 장의 움직이는 GIF로.

```bash
wonjang 움짤 1.png 2.png 3.png --초 0.5   # 0.5초 간격 움짤
wonjang 움짤 *.png --폭 480 --출력 demo.gif
```

비율 유지로 같은 크기 프레임을 만들어 왜곡 없이, 무한반복 GIF로. (이미지 줄이기 `wonjang 이미지 사진.jpg --폭 1280`, 이어붙이기 `wonjang 이미지이어붙이기`도 있어요.)
